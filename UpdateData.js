...
async function buildEquipmentData() {
	try {
		...
		for (const entry of baseEquipmentConfig) {
			...
			equipmentContent["Family"] = entry["family"];
...
async function buildWeaponData() {
	try {
		...
		for (const entry of baseWeaponConfig) {
			...
			weaponContent["Family"] = entry["family"];
			...
			if (entry["passives"]) {
				...
				for (const skillID of Object.values(entry["passives"])) {
					const skillDetail = {};
					skillDetail["SkillCodename"] = skillID;
		...
		for (const entry of baseMonsterConfig) {
			...
			monsterContent["FullName"] = monsterContent["Name"]
				? monsterContent["Name2"]
					...
					: monsterContent["Name"]
				: entryID;
			monsterContent["Family"] = entry["family"];
			...
			if (entry["leaderBuff"]) {
				...
				monsterContent["LeaderSkill"]["SkillCodename"] = skillID;
...
function setExtraEvoCount(jsonData) {
	for (const id in jsonData) {
		...
		const familyName = jsonData[id]["Family"];
		const numInFamily = Number(id.replace(familyName, ""));
		if (!numInFamily) continue;
